...
DinnerMatchSocial is a React Native mobile application built with Expo. The project uses a monorepo structure with the main application located in `apps/frontend/`.
...
cd apps/frontend && bun ios      # Start iOS simulator
cd apps/frontend && bun android  # Start Android emulator
cd apps/frontend && bun web      # Start web version
```
...
This is an Expo Router-based React Native application with file-based routing. Key architectural elements:

### Navigation Structure
- **Root Layout** (`app/_layout.tsx`): Manages the root navigation stack, theme provider, and font loading
- **Tab Navigation** (`app/(tabs)/_layout.tsx`): Implements bottom tab navigation with two main tabs
- **Modal Presentation**: Supports modal screens via stack navigation

### Routing System
- Uses Expo Router v6 with file-based routing
- Typed routes are enabled for better TypeScript support
- Initial route is set to `(tabs)` directory
- Supports both tab-based and modal navigation patterns

### Theming
- Implements automatic dark/light theme switching based on device settings
- Uses React Navigation themes (DefaultTheme/DarkTheme)
- Color constants defined in `constants/Colors.ts` for consistent theming

### Component Structure
- **Shared Components** (`components/`): Reusable UI components
  - Theme-aware components (`Themed.tsx`)
  - Platform-specific implementations (`.web.ts` extensions for web-specific code)
  - Custom hooks for color scheme and client-only values

### Platform Support
- iOS, Android, and Web targets
- Uses React Native v0.81.5 with the new architecture enabled
- Web bundler configured to use Metro with static output

## TypeScript Configuration

- Strict mode enabled
- Path alias configured: `@/*` maps to the frontend root directory
- Includes expo type definitions and environment types
